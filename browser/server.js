 ...
 * Supports two modes:
 * - 'secure-openclaw': Managed isolated browser with dedicated profile
 * - 'chrome': CDP connection to existing Chrome instance
 */
export default class BrowserServer {
  constructor(config = {}) {
    this.mode = config.mode || 'secure-openclaw'
    this.config = config
    this.browser = null
    this.context = null
    this.page = null
  ...
  async startManagedBrowser() {
    const browserConfig = this.config['secure-openclaw'] || {}
    let userDataDir = browserConfig.userDataDir || '~/.secure-openclaw-browser-profile'
    ...
    console.log('[BrowserServer] Launching secure-openclaw browser with profile:', userDataDir)
    ...
    console.log('[BrowserServer] Secure-OpenClaw browser started')
  }
  ...
  async stop() {
    ...
    if (this.context && this.mode === 'secure-openclaw') {
